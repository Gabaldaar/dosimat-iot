...
async def enviar_alerta_directa_nube_async(tipo_evento, mensaje):
    """Envía la alerta directamente a Google Cloud Functions sin intermediarios"""
    global wifi_conectado
    if not wifi_conectado:
        return

    # Liberar memoria RAM antes de abrir el socket
    gc.collect()
    s = None
    try:
        ...
        resp = s.readline()
        print(f"[DIRECT_PUSH] Alerta '{tipo_evento}' enviada directo a Google Cloud ({resp.decode().strip()})")
    except Exception as e:
        print(f"[DIRECT_PUSH] Aviso envio directo ({e})")
    finally:
        if s:
            try:
                s.close()
